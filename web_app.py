...
def rerun_black_trace_in_guided_band(
    curve_mask,
    guide_xs,
    scale_min,
    scale_max,
    ...
    """Rerun black DP inside a narrow corridor around the first trace.

    The first pass usually knows the right neighborhood even when a few rows
    jump onto grid/text. Constraining the second DP pass to a smoothed guide
    band keeps distant dark junk from competing with the actual curve.
    """
    if curve_mask is None or guide_xs is None:
        return guide_xs
    if not hasattr(curve_mask, "shape") or len(curve_mask.shape) != 2:
        return guide_xs
    if not hasattr(guide_xs, "size") or guide_xs.size < 3:
        return guide_xs
    ...
    xs_ref = guide_xs.copy().astype(np.float32)
    n = min(h, xs_ref.size)
    valid = np.isfinite(xs_ref[:n])
    if int(valid.sum()) < max(8, int(n * 0.10)):
        return xs_ref

    try:
        filled = pd.Series(xs_ref[:n]).interpolate(
            method="linear",
            limit_direction="both",
        ).ffill().bfill()
        if filled.isna().any():
        ...
        guide_win = min(guide_win, n if n % 2 else n - 1)
        if guide_win >= 5:
            guide = filled.rolling(
                guide_win,
                min_periods=max(3, min(9, guide_win // 3)),
                center=True,
            ).median().bfill().ffill().to_numpy(dtype=np.float32)
        else:
            guide = filled.to_numpy(dtype=np.float32)
    except Exception:
        return xs_ref

    radius = band_radius
    if radius is None:
        radius = max(10, min(30, int(round(w * 0.055))))
    radius = max(6, int(radius))

    guided_mask = np.zeros_like(curve_mask, dtype=np.uint8)
    for y in range(n):
        gx = guide[y]
        ...
        guided_mask[y, x0:x1] = curve_mask[y, x0:x1]

    if cv2.countNonZero(guided_mask[:n]) <= max(12, n // 8):
        return xs_ref

    try:
        guided_step = max(6, min(int(max_step), max(14, int(round(radius * 1.6)))))
        xs_band, _conf_band = trace_curve_with_dp(
            guided_mask,
            scale_min=scale_min,
            scale_max=scale_max,
            curve_type=curve_type,
            max_step=guided_step,
            smooth_lambda=smooth_lambda,
            curv_lambda=curv_lambda,
            hot_side=hot_side,
        )
    ...
    if xs_band is None or not hasattr(xs_band, "size") or xs_band.size == 0:
        return xs_ref

    xs_band = xs_band.astype(np.float32, copy=True)
    n_band = min(n, xs_band.size)
    band_valid = np.isfinite(xs_band[:n_band])
    valid_ratio = float(band_valid.mean()) if n_band else 0.0
    if valid_ratio < 0.28:
        return xs_ref

    try:
        band_filled = pd.Series(xs_band[:n_band]).interpolate(
            method="linear",
            limit_direction="both",
        ).ffill().bfill().to_numpy(dtype=np.float32)
    except Exception:
        band_filled = xs_band[:n_band]

    if not np.isfinite(band_filled).any():
        return xs_ref

    guide_cmp = guide[:n_band]
    finite_cmp = np.isfinite(band_filled) & np.isfinite(guide_cmp)
    if not np.any(finite_cmp):
        return xs_ref
    if float(np.nanmedian(np.abs(band_filled[finite_cmp] - guide_cmp[finite_cmp]))) > float(radius * 0.95):
        return xs_ref

    prob = guided_mask.astype(np.float32) / 255.0
    rows = np.arange(n_band, dtype=np.int32)
    cols = np.clip(np.round(band_filled[:n_band]).astype(np.int32), 0, w - 1)
    support = prob[rows, cols]
    if float(np.nanmean(support[finite_cmp])) < 0.010:
        return xs_ref

    xs_out = xs_ref.copy()
    xs_out[:n_band] = band_filled[:n_band]
    return xs_out
...
def digitize():
    ...
    for c in curves:
        ...
        # AND DISABLE EXTRA REFINEMENTS which cause the zig-zag snapping.
            ...
            # Guided rerun: the first pass usually identifies the right lane,
            # but distant grid/text can still win isolated rows. Re-run DP
            # inside a smoothed corridor around that first trace before the
            # local continuous-line cleanup.
            try:
                xs = rerun_black_trace_in_guided_band(
                    mask,
                    xs,
                    scale_min=left_value,
                    scale_max=right_value,
